...
public class GooView extends View {
    ...
    private void init(Context context) {
        stickCircleRadius = dip2Dimension(10.0f, context);
        dragCircleRadius = dip2Dimension(10.0f, context);
        stickCircleMinRadius = dip2Dimension(3.0f, context);
        mMaxDistance = dip2Dimension(70.0f, context);
        mResetDistance = dip2Dimension(40.0f, getContext());
    ...
    public boolean onTouchEvent(MotionEvent event) {
        switch (MotionEventCompat.getActionMasked(event)) {
            case MotionEvent.ACTION_DOWN: {
                //关闭侧滑
//                EventBus.getDefault().post(new IsSwipeEnableEvent(false, getPosition()));
                isOutOfRange = false;
            ...
            case MotionEvent.ACTION_UP: {
                //手指离开后，开启侧滑
//                EventBus.getDefault().post(new IsSwipeEnableEvent(true, getPosition()));
                handleActionUp();
